...
vortex::NDIOutput::NDIOutput(const vortex::Graphics& gfx, SerializedProperties props)
    ...
    , _audio_buffer(vortex::AudioFormat{ .data_format = vortex::Float32Planar,
                    ...
                    })
{
    ...
    _audio_samples.resize(samples_per_frame *
                          2); // Reserve space for 1 second of stereo float samples at 48kHz
}
        ...
        if (_swapchain.Resize(gfx, window_size.x, window_size.y)) {
            ...
            for (size_t i = 0; i < textures.size(); ++i) {
                ...
                _render_targets[i] = gfx.GetDevice().CreateRenderTarget(
                        result,
                        textures[i],
                        wis::RenderTargetDesc{ .format = format });
                if (!vortex::success(result)) {
...
void vortex::NDIOutput::Throttle() const
{
    ...
    if (res.status == wis::Status::Timeout) {
        vortex::warn("NDIOutput: Timeout while waiting for fence in Throttle(). The GPU may be "
                     "unresponsive.");
...
bool vortex::NDIOutput::EvaluateAudio()
{
    ...
    audio_probe.audio_sample_rate = audio_sample_rate; // 48 kHz (Used to determine if input is
                                                       // needed)
    ...
    if (!audio_probe.audio_data.empty()) {
        ...
        _audio_buffer.WritePlanar(
                std::span{ audio_probe.audio_data }); // Write the audio data to the buffer
