...
class EngineBridge {
  ...
  on(event: string, handler: EngineEventHandler) {
    if (this.native?.on) {
      const off = this.native.on(event, handler)
      if (off) return off
    }
    ...
    if (event === 'recents:updated' && typeof window !== 'undefined') {
      ...
      window.addEventListener(RECENTS_EVENT, listener)
      return () => window.removeEventListener(RECENTS_EVENT, listener)
    }
    ...
    if (event === 'lastProject:updated' && typeof window !== 'undefined') {
      ...
      window.addEventListener(LAST_PROJECT_EVENT, listener)
      return () => window.removeEventListener(LAST_PROJECT_EVENT, listener)
    }
